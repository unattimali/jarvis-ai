programming_jokes = [

    {
        "setup": "Why do programmers prefer dark mode?",
        "punchline": "Because light attracts bugs."
    },

    {
        "setup": "Why did the Java developer wear glasses?",
        "punchline": "Because he couldn't C."
    },

    {
        "setup": "Why was the Python programmer calm?",
        "punchline": "He always handled exceptions."
    },

    {
        "setup": "Why don't programmers like nature?",
        "punchline": "It has too many bugs."
    },

    {
        "setup": "Why was the computer cold?",
        "punchline": "It left its Windows open."
    },

    {
        "setup": "Why did the programmer quit his job?",
        "punchline": "Because he didn't get arrays."
    },

    {
        "setup": "Why do Python programmers never panic?",
        "punchline": "They just try and except."
    },

    {
        "setup": "How many programmers does it take to change a light bulb?",
        "punchline": "None. That's a hardware problem."
    },

    {
        "setup": "Why did the function break up with the loop?",
        "punchline": "Because it needed some space."
    },

    {
        "setup": "Why was the programmer broke?",
        "punchline": "He used up all his cache."
    },

    {
        "setup": "Why do programmers love coffee?",
        "punchline": "Because Java keeps them awake."
    },

    {
        "setup": "Why did the coder go to therapy?",
        "punchline": "Too many unresolved issues."
    },

    {
        "setup": "Why couldn't the programmer sleep?",
        "punchline": "He forgot to close his tabs."
    },

    {
        "setup": "Why was the HTML page sad?",
        "punchline": "It couldn't find its body."
    },

    {
        "setup": "Why did CSS go to the gym?",
        "punchline": "To improve its flex."
    },

    {
        "setup": "Why did JavaScript get detention?",
        "punchline": "It kept acting up."
    },

    {
        "setup": "Why don't programmers trust stairs?",
        "punchline": "They're always up to something."
    },

    {
        "setup": "What do you call a programmer from Finland?",
        "punchline": "Nerdic."
    },

    {
        "setup": "Why was Git always calm?",
        "punchline": "Because it knew how to commit."
    },

    {
        "setup": "Why did the database blush?",
        "punchline": "Someone joined its tables."
    }

]

dad_jokes = [

    "Why don't eggs tell jokes? They'd crack each other up.",

    "I only know 25 letters of the alphabet. I don't know Y.",

    "Why did the scarecrow win an award? Because he was outstanding in his field.",

    "I'm reading a book on anti-gravity. It's impossible to put down.",

    "Did you hear about the restaurant on the moon? Great food, no atmosphere.",

    "Why don't skeletons fight each other? They don't have the guts.",

    "Why did the bicycle fall over? Because it was two-tired.",

    "What do you call fake spaghetti? An impasta.",

    "Why can't your nose be 12 inches long? Because then it'd be a foot.",

    "What did one wall say to the other? I'll meet you at the corner.",

    "Why did the golfer bring two pairs of pants? In case he got a hole in one.",

    "Why are elevator jokes so good? They work on many levels.",

    "What kind of tree fits in your hand? A palm tree.",

    "Why was six afraid of seven? Because seven eight nine.",

    "Why did the tomato blush? Because it saw the salad dressing.",

    "What do you call cheese that isn't yours? Nacho cheese.",

    "How do cows stay up to date? They read the moos-paper.",

    "Why do bees have sticky hair? They use honeycombs.",

    "What did the ocean say to the beach? Nothing. It just waved.",

    "What do you call a bear with no teeth? A gummy bear."

]
riddles = [

{
"question":"What has keys but can't open locks?",
"answer":"A piano."
},

{
"question":"What has hands but can't clap?",
"answer":"A clock."
},

{
"question":"The more you take, the more you leave behind. What am I?",
"answer":"Footsteps."
},

{
"question":"What gets wetter the more it dries?",
"answer":"A towel."
},

{
"question":"What has one eye but cannot see?",
"answer":"A needle."
},

{
"question":"What can travel around the world while staying in one corner?",
"answer":"A stamp."
},

{
"question":"What comes down but never goes up?",
"answer":"Rain."
},

{
"question":"What has many teeth but cannot bite?",
"answer":"A comb."
},

{
"question":"What begins with T, ends with T and has T inside?",
"answer":"A teapot."
},

{
"question":"What has a neck but no head?",
"answer":"A bottle."
},

{
"question":"What is full of holes but still holds water?",
"answer":"A sponge."
},

{
"question":"What can you catch but not throw?",
"answer":"A cold."
},

{
"question":"What has four wheels and flies?",
"answer":"A garbage truck."
},

{
"question":"What has legs but doesn't walk?",
"answer":"A table."
},

{
"question":"What has an eye but cannot see?",
"answer":"A hurricane."
},

{
"question":"What kind of room has no doors or windows?",
"answer":"A mushroom."
},

{
"question":"What goes up and never comes down?",
"answer":"Your age."
},

{
"question":"What is always in front of you but can't be seen?",
"answer":"The future."
},

{
"question":"I speak without a mouth and hear without ears. What am I?",
"answer":"An echo."
},

{
"question":"What can run but never walks?",
"answer":"Water."
}

]
pickup_lines = [

"Are you Wi-Fi? Because I'm really feeling a connection.",
"Are you a keyboard? Because you're just my type.",
"Do you believe in love at first sight, or should I walk by again?",
"Are you Google? Because you have everything I'm searching for.",
"If beauty were time, you'd be eternity.",
"You must be made of copper and tellurium because you're Cu-Te.",
"Are you a magician? Whenever I look at you, everyone else disappears.",
"Is your name Python? Because you've got me hooked.",
"You stole my heart, but I'll let you keep it.",
"Can I borrow a map? I keep getting lost in your eyes."

]
roasts = [

"You're as useful as a screen door on a submarine.",
"You're the reason the shampoo bottle has instructions.",
"If laziness were a sport, you'd still come second.",
"You bring everyone so much joy... when you leave the room.",
"Your secrets are always safe with me. I never even listen.",
"I'd explain it to you, but I left my crayons at home.",
"You have something on your chin... no, the third one.",
"You're proof that evolution can go in reverse.",
"If I wanted to hear from someone useless, I'd unplug my Wi-Fi.",
"You have the confidence of someone who's never been told they're wrong.",
"You're like a cloud. When you disappear, it's a beautiful day.",
"You couldn't pour water out of a boot if the instructions were on the heel.",
"Your Wi-Fi signal has more personality than you.",
"You're not stupid—you just have really bad luck thinking.",
"I've seen smarter conversations in YouTube comment sections.",
"You're like Internet Explorer... everyone has moved on.",
"You're as sharp as a marble.",
"You have the charm of a software update.",
"If ignorance is bliss, you must be the happiest person alive.",
"You couldn't organize two files into one folder."

]
truths = [

"What's your biggest fear?",
"Have you ever lied to your best friend?",
"What's the most embarrassing thing you've ever done?",
"What's one secret you've never told anyone?",
"Who was your first crush?",
"Have you ever cheated on a test?",
"What's your worst habit?",
"What's something you're glad your parents don't know?",
"What's your biggest regret?",
"What's the weirdest dream you've ever had?",
"What's your guilty pleasure?",
"If you could change one thing about yourself, what would it be?",
"Have you ever pretended to be sick to skip school or work?",
"What's the funniest lie you've ever told?",
"Who in your contacts would you never answer a call from?",
"What's something you've done that you're still embarrassed about?",
"If you became invisible for one day, what would you do?",
"Have you ever stalked someone on social media?",
"What's the strangest thing you've searched on Google?",
"What's your biggest weakness?"

]
dares = [

"Sing your favorite song loudly.",
"Do 20 jumping jacks.",
"Talk like a robot for the next minute.",
"Dance for 30 seconds without music.",
"Tell a joke and don't laugh.",
"Do your best animal impression.",
"Say the alphabet backwards as far as you can.",
"Take a funny selfie.",
"Balance a book on your head for one minute.",
"Pretend you're a news reporter for one minute.",
"Speak in rhyme for the next two minutes.",
"Try to touch your nose with your tongue.",
"Act like your favorite movie character.",
"Walk like a penguin for one minute.",
"Compliment the first person you see.",
"Do five push-ups.",
"Try to whistle a song.",
"Read the last text message you received.",
"Spin around 10 times and walk straight.",
"Make the funniest face you can."

]
would_you_rather = [

"Would you rather fly or become invisible?",
"Would you rather live without the internet or without music?",
"Would you rather always be 10 minutes late or 20 minutes early?",
"Would you rather eat only pizza forever or only burgers forever?",
"Would you rather travel to the past or the future?",
"Would you rather have unlimited money or unlimited time?",
"Would you rather never use social media again or never watch YouTube again?",
"Would you rather own a Ferrari or a private jet?",
"Would you rather be famous or be rich?",
"Would you rather read minds or see the future?",
"Would you rather never sleep or never eat?",
"Would you rather have a pet dragon or a pet dinosaur?",
"Would you rather live on Mars or under the ocean?",
"Would you rather know every language or play every instrument?",
"Would you rather be able to teleport or time travel?",
"Would you rather always win every game or always know the right answer?",
"Would you rather become a superhero or a billionaire?",
"Would you rather have unlimited free food or unlimited free travel?",
"Would you rather never use a phone again or never use a computer again?",
"Would you rather be the smartest person alive or the funniest person alive?"

]
motivations = [

"Believe in yourself. Every expert was once a beginner.",
"Small progress is still progress.",
"Your only limit is your mindset.",
"Success doesn't come from what you do occasionally, it comes from what you do consistently.",
"Don't wait for opportunity. Create it.",
"Dream big. Start small. Act now.",
"Push yourself because no one else is going to do it for you.",
"Failure is not the opposite of success. It is part of success.",
"Every day is a new beginning.",
"Stay positive, work hard, make it happen.",
"The best time to start was yesterday. The next best time is now.",
"You are stronger than you think.",
"Great things take time.",
"Discipline beats motivation.",
"Never stop learning.",
"The harder you work, the luckier you get.",
"Success starts with self-belief.",
"Don't compare yourself to others. Compare yourself to yesterday's you.",
"Keep going. You're closer than you think.",
"Every challenge is an opportunity to grow."

]
quotes = [

"Success is not final, failure is not fatal: it is the courage to continue that counts. — Winston Churchill",

"The future depends on what you do today. — Mahatma Gandhi",

"Believe you can and you're halfway there. — Theodore Roosevelt",

"Dream big and dare to fail. — Norman Vaughan",

"The only way to do great work is to love what you do. — Steve Jobs",

"Do what you can, with what you have, where you are. — Theodore Roosevelt",

"It always seems impossible until it's done. — Nelson Mandela",

"Don't watch the clock; do what it does. Keep going. — Sam Levenson",

"Opportunities don't happen. You create them. — Chris Grosser",

"Success usually comes to those who are too busy to be looking for it. — Henry David Thoreau",

"Everything you've ever wanted is on the other side of fear. — George Addair",

"Be yourself; everyone else is already taken. — Oscar Wilde",

"If opportunity doesn't knock, build a door. — Milton Berle",

"Hard work beats talent when talent doesn't work hard. — Tim Notke",

"Your limitation—it's only your imagination.",

"Great things never come from comfort zones.",

"Don't stop until you're proud.",

"The best view comes after the hardest climb.",

"Every accomplishment starts with the decision to try.",

"Stay hungry. Stay foolish. — Steve Jobs"

]
compliments = [

"You're smarter than you think.",
"You have an amazing sense of humor.",
"Your creativity is inspiring.",
"You make the world a better place.",
"You have incredible potential.",
"You brighten people's day.",
"You are capable of amazing things.",
"Your kindness is your superpower.",
"You have a beautiful mind.",
"You deserve happiness.",
"You are stronger than yesterday.",
"Your confidence is growing every day.",
"You never stop learning, and that's admirable.",
"You have excellent taste.",
"You are more talented than you realize.",
"You have a unique perspective.",
"People appreciate having you around.",
"You are doing better than you think.",
"You are one of a kind.",
"You have the power to achieve your dreams."

]
brain_teasers = [

"What comes once in a minute, twice in a moment, but never in a thousand years?\n\nAnswer: The letter M.",

"I speak without a mouth and hear without ears. I have no body, but I come alive with wind.\n\nAnswer: An echo.",

"The more you take, the more you leave behind.\n\nAnswer: Footsteps.",

"What has many keys but can't open a single lock?\n\nAnswer: A piano.",

"What gets wetter the more it dries?\n\nAnswer: A towel.",

"I have branches but no fruit, trunk or leaves.\n\nAnswer: A bank.",

"What can travel around the world while staying in one corner?\n\nAnswer: A stamp.",

"What has hands but cannot clap?\n\nAnswer: A clock.",

"What has one eye but cannot see?\n\nAnswer: A needle.",

"What has a neck but no head?\n\nAnswer: A bottle.",

"What begins with T, ends with T, and has T inside?\n\nAnswer: A teapot.",

"What is full of holes but still holds water?\n\nAnswer: A sponge.",

"What belongs to you but others use it more than you do?\n\nAnswer: Your name.",

"What comes down but never goes up?\n\nAnswer: Rain.",

"What can you catch but not throw?\n\nAnswer: A cold.",

"What has cities but no houses, forests but no trees, rivers but no water?\n\nAnswer: A map.",

"What goes up and never comes down?\n\nAnswer: Your age.",

"What has legs but doesn't walk?\n\nAnswer: A table.",

"What has words but never speaks?\n\nAnswer: A book.",

"What invention lets you look right through a wall?\n\nAnswer: A window."

]
magic_8_ball = [

"🟢 It is certain.",
"🟢 Definitely yes.",
"🟢 Without a doubt.",
"🟢 Absolutely!",
"🟢 You can count on it.",
"🟢 Signs point to yes.",
"🟢 The stars agree.",
"🟢 Yes, go for it!",
"🟢 Everything says yes.",
"🟢 Success is waiting for you.",

"🟡 Maybe.",
"🟡 Ask again later.",
"🟡 The future is unclear.",
"🟡 Better not tell you now.",
"🟡 It could go either way.",
"🟡 Wait a little longer.",
"🟡 Trust your instincts.",
"🟡 Time will tell.",
"🟡 Not enough information.",
"🟡 Focus and ask again.",

"🔴 Don't count on it.",
"🔴 Very doubtful.",
"🔴 Outlook not so good.",
"🔴 My answer is no.",
"🔴 Chances are low.",
"🔴 Better avoid it.",
"🔴 I wouldn't recommend it.",
"🔴 It seems unlikely.",
"🔴 Probably not.",
"🔴 The signs say no."

]
excuses = [

"My Wi-Fi stopped working at the worst possible moment.",
"My alarm didn't ring this morning.",
"My laptop decided to update itself.",
"My cat walked across the keyboard and closed everything.",
"The internet was slower than a turtle.",
"My charger suddenly stopped working.",
"I accidentally deleted the file.",
"My phone battery died.",
"There was a power cut in my area.",
"My browser crashed unexpectedly.",
"My computer froze for 20 minutes.",
"I forgot to save my work.",
"My keyboard wasn't typing properly.",
"My mouse stopped responding.",
"My dog hid my notebook.",
"I couldn't find the right file.",
"My USB drive wasn't detected.",
"My notebook got wet.",
"I lost track of time.",
"I was stuck in traffic... even in my imagination.",
"My brain went into airplane mode.",
"I was helping my family.",
"I wasn't feeling well.",
"My room was too distracting.",
"My little cousin wouldn't stop bothering me.",
"My coffee wasn't strong enough.",
"I was waiting for inspiration.",
"I underestimated how long it would take.",
"My calendar betrayed me.",
"I thought today was Sunday."

]
excuse_endings = [

"😂 Hopefully they believe you!",
"🙈 Use this wisely!",
"😅 Don't blame Jarvis if it doesn't work!",
"🤫 Keep a straight face!",
"🤣 Good luck explaining that!",
"😎 That's my best excuse today."

]
gamer_challenges = [

# 💻 Coding
"💻 Solve one Python problem on HackerRank.",
"💻 Build a mini project in 30 minutes.",
"💻 Learn one new Python function today.",
"💻 Debug a piece of code without using Google.",
"💻 Write a program with fewer than 20 lines.",

# 📚 Study
"📚 Study for 25 minutes without touching your phone.",
"📚 Revise one complete chapter today.",
"📚 Teach someone a topic you've learned.",
"📚 Read 10 pages of a book.",
"📚 Learn five new English words.",

# 💪 Fitness
"💪 Do 20 push-ups.",
"💪 Walk for 15 minutes.",
"💪 Drink two glasses of water right now.",
"💪 Stretch your body for 5 minutes.",
"💪 Do 30 jumping jacks.",

# 🎮 Gaming
"🎮 Win your next match without raging.",
"🎮 Play one game without using your favorite weapon.",
"🎮 Try a completely different strategy.",
"🎮 Help a beginner player.",
"🎮 Finish one level without dying.",

# 🌍 Fun
"🌍 Compliment someone today.",
"🌍 Stay away from social media for 30 minutes.",
"🌍 Listen to your favorite song.",
"🌍 Watch a TED Talk.",
"🌍 Smile at three people today."

]
foods = [

# 🍛 Indian
"🍛 Paneer Butter Masala with Naan",
"🍛 Dal Tadka with Jeera Rice",
"🍛 Chole Bhature",
"🍛 Rajma Chawal",
"🍛 Veg Biryani",
"🍛 Pav Bhaji",
"🍛 Masala Dosa",
"🍛 Idli Sambhar",
"🍛 Aloo Paratha with Curd",
"🍛 Gujarati Thali",

# 🍕 Fast Food
"🍕 Margherita Pizza",
"🍔 Veg Burger",
"🌮 Tacos",
"🌯 Burrito",
"🍟 French Fries",
"🌭 Hot Dog",
"🥪 Grilled Sandwich",
"🍝 White Sauce Pasta",
"🍜 Hakka Noodles",
"🍲 Manchurian with Fried Rice",

# 🥗 Healthy
"🥗 Green Salad",
"🥣 Oats",
"🥛 Fruit Smoothie",
"🍎 Apple and Peanut Butter",
"🥑 Avocado Toast",
"🍉 Mixed Fruits",
"🥜 Dry Fruits",
"🥗 Sprouts Salad",
"🍌 Banana Shake",
"🥚 Boiled Eggs",

# 🍰 Desserts
"🍨 Vanilla Ice Cream",
"🍫 Chocolate Brownie",
"🍰 Cheesecake",
"🍩 Donuts",
"🍪 Cookies",
"🍮 Gulab Jamun",
"🍧 Kulfi",
"🍰 Black Forest Cake",
"🍨 Chocolate Ice Cream",
"🍫 Dairy Milk Chocolate",

# ☕ Drinks
"☕ Coffee",
"🧋 Bubble Tea",
"🥤 Cold Coffee",
"🍹 Mango Shake",
"🍋 Lemon Juice",
"🥛 Chocolate Milk",
"🧃 Orange Juice",
"🍵 Green Tea",
"🥭 Lassi",
"🥤 Fresh Lime Soda"
"Pani puri"

]
food_endings = [

"😋 Enjoy your meal!",
"🍽 Bon Appétit!",
"🤤 Sounds delicious!",
"👨‍🍳 Chef Jarvis recommends this!",
"🥳 Hope you love it!",
"🍴 Time to eat!",
"❤️ Food always makes everything better."

]